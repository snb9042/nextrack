...
Usage:
  python icloud_bridge.py <email> <password>        # first run: may prompt for 2FA
  python icloud_bridge.py <email> <password> <code> # submit 2FA code
...
def main():
    args = sys.argv[1:]
    if len(args) < 2:
        print(json.dumps({"ok": False, "error": "Usage: icloud_bridge.py <email> <password> [2fa_code]"}))
        sys.exit(1)

    email    = args[0]
    password = args[1]
    code_2fa = args[2] if len(args) > 2 else None
    ...
    if api.requires_2fa:
        if code_2fa:
            ...
            print(json.dumps({
                "ok": False,
                "needs2FA": True,
                "error": "2FA required — run again with the 6-digit code from your iPhone as third argument"
            }))
            sys.exit(0)
